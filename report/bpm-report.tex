...
We illustrate the process of anti-unification using the expressions \texttt{(+ (+ 2 2) (- 2 5))} and \texttt{(+ (- 2 3) 4)}.
We first compare the root of the trees and make sure that we have lists of the same size. In this case, they are both of size three, therefore we have matching roots and a partial match of \texttt{(* * *)} where the \texttt{*}s are yet to be determined.
Now we recursively attempt to match the three subexpressions \texttt{+} with \texttt{+}, \texttt{(+ 2 2)} with \texttt{(- 2 3)}, and \texttt{(- 2 5)} with \texttt{4}.
Since \texttt{+} and \texttt{+} are identical primitives, they match, and our partial match is now \texttt{(+ * *)}.
Comparing \texttt{(+ 2 2)} and \texttt{(- 2 3)}, we see that they are both lists of size 3 and therefore they match, giving us a total partial match of \texttt{(+ (* * *) *)}.
Again, we recursively match subexpressions of \texttt{(+ 2 2)} and \texttt{(- 2 3)}, i.e., \texttt{+} against \texttt{-}, \texttt{2} against \texttt{2}, and \texttt{2} against \texttt{3}.
Since \texttt{+} and \texttt{-} are primitives that do not match, we replace them with a variable. Our partial match is now \texttt{(+ (V1 * *) *)}.
After comparing \texttt{2} to \texttt{2} and \texttt{2} to \texttt{3}, we have \texttt{(+ (V1 2 V2) *)}.
In the final comparison of \texttt{(- 2 5)} and \texttt{4}, we find that there is no match since \texttt{(- 2 5)} is a list and \texttt{4} is a primitive. Therefore, the final result of our anti-unification is \texttt{(+ (V1 2 V2) V3)}.
