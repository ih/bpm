...
\caption{The expression \texttt{(+ (+ 2 2) (- 2 5))} represented as a tree. Anti-unification recursively finds common subtrees between two syntax trees.}
...
If two variables take on the same value in all of their applications, we consider them identical. In order to allow for more compressive proposals, we also consider any two numbers to be equal, relying on the resulting likelihood to filter out models that do not explain the data.
