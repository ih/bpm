...
(define program (sexpr->program '(let () (define F1 (lambda (V1) (node V1))) (F1 (F1 1)))))
...
(let* ([program (sexpr->program '(let () (define F1 (lambda (V1) (node V1))) (F1 (F1 1))))]
       [abstraction (make-named-abstraction 'F1 '(node V1) '(V1))])
  (check (remove-abstraction-variable program abstraction 'V1) => (make-named-abstraction 'F1 '((lambda (V1) (node V1)) ((uniform-draw (list (lambda () (F1 1)) (lambda () 1))))) '())))
...
(let* ([program (sexpr->program '(let () (define F1 (lambda (V1) (node V1))) (F1 (F1 1))))]
       [abstraction (make-named-abstraction 'F1 '(node V1) '(V1))]
       [new-abstraction (remove-abstraction-variable program abstraction 'V1)]
...
;;;internalize-arguments
  (let* ([correct-abstraction (make-named-abstraction 'F1 '(let ([V1 ((uniform-draw (list (lambda () (F1)) (lambda () 1))))]) (node V1)) '())]
         [correct-program (make-program (list correct-abstraction) '(F1))])
    (check (internalize-arguments program #t)
          =>
