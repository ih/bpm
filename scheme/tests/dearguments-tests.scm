...
(define noisy-number-target-abstraction (make-named-abstraction 'F1 '(node V1) '(V1)))
(define noisy-number-program (make-program (list noisy-number-target-abstraction) '(node (F1 1) (F1 1.1))))
;;;noisy-number-replacement
(check (noisy-number-replacement (find-variable-instances noisy-number-program noisy-number-target-abstraction 'V1)) => `(gaussian ,(mean (list 1 1.1)) ,(variance (list 1 1.1))))

;;;noisy-number test
(define noisy-number-dearguments (make-dearguments-transformation noisy-number-replacement))
(let* ([correct-abstraction (make-named-abstraction 'F1 `((lambda (V1) (node V1)) (gaussian ,(mean (list 1 1.1)) ,(variance (list 1 1.1)))) '())]
